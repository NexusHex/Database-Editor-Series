...
def show_patches():
  ...
  >>>>>>>>>>>>> Renamed variable 'data' to 'data_store' to avoid readability issues (minor update)
  >>>>>>>>>>>>> User can now view all patch notes within command 'patches' or '_p' when the Editor prompts you to pick an option (minor update)
